...
class UploadTXTForm(forms.Form):
    txtFile = ExtFileField(ext_whitelist=('.txt','.txt'),widget=forms.FileInput(attrs={'class':'custom-file-input'})) 
...
class UploadWAVForm(forms.Form):
    wavFile = ExtFileField(ext_whitelist=('.wav','.wav'),widget=forms.FileInput(attrs={'class':'custom-file-input'}))
